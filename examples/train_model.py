...
def train_model(audio_dirs:list[str],
                seg_dirs:list[str],
                model_dir:str,
                max_vocs:int=5000,
                context_len=0.25,
    ...
    assert len(audio_dirs) == len(seg_dirs), print("Need the same number of audio dirs as segment dirs!")
    ...
    for audio_dir, seg_dir in zip(audio_dirs,seg_dirs):
        audio,sr = get_segmented_audio(audio_dir,
                                       seg_dir,
        ...
        chunks += audio
    dt=1/sr
    dataloaders = get_loaders(chunks,
                              num_workers=num_workers,
                              batch_size=batch_size,
                              train_size=0.6,cv=True,
                              seed=seed,
                              dt=dt)
