...
def run_model(
    ...
    opt = Adam(list(model.parameters()) + list(filt.parameters()), lr=1e-3)
    scheduler = ReduceLROnPlateau(opt, factor=0.75, patience=5, min_lr=1e-10)

    model_path_full = (
        model_path
        + f"/kernelborous_{voctype}_trendfiltering_{trend_level}_alpha_{alpha}_kernel_{kernel_type}_nkernels_{n_kernels}"
    )
