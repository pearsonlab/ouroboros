...
def analyze_model(model_path,audio_path,seg_path = '',\
                  context_len=0.3,seg_filetype='.txt',max_pairs=1000,audio_type='.wav',\
                    kernel_type='gauss',n_kernels=10,\
                        plot_fig = True,plot_fncs=True,
                        nPairs=20):

    if kernel_type == 'gauss':
        kernel = simpleGaussModule(nTerms=n_kernels,device='cuda',xdim=1,z_dim=4,activation=lambda x: x)
    else:
        kernel = polyModule(nTerms=n_kernels,device='cuda',x_dim=1,z_dim=4,activation = lambda x: x)
    ...
    audios,sr = get_segmented_audio(audio_path,seg_path,envelope=False,context_len=context_len,\
                                    audio_type=audio_type,seg_type=seg_filetype,max_pairs=max_pairs)
