...
def run_experiments(
    ...
    if dir_undir:
        ...
        audios, sr = get_segmented_audio(
            dud_aud,
            dud_seg,
            audio_subdir="[U,D]*/double_denoised",
            seg_subdir="[U,D]*",
            envelope=False,
            context_len=0.15,
        ...
        # print(sr)
        dls = get_loaders(
            np.vstack(audios), cv=True, train_size=0.6, seed=seed, batch_size=batch_size
        )

        # print(np.amax(audios),np.amin(audios))
        dud_model = model_cv_lambdas(
            dls,
            1 / sr,
            nEpochs=nEpochs,
            model_path=dir_undir_modelpath,
            n_layers=3,
            expand_factor=8,
            n_kernels=n_kernels,
            tau=1 / sr,
        ...
        dud_r2s, dud_best, dud_resids, dud_spec_ratio, dud_specs, dud_ext = (
            full_eval_model(
                dud_model,
                dls,
                audios,
    ...
    if pupil_tutor:
        ...
        pt_model = model_cv_lambdas(
            dls,
            1 / sr,
            nEpochs=200,
            model_path=pupil_tutor_modelpath,
            n_layers=3,
            expand_factor=8,
            n_kernels=n_kernels,
            tau=1 / sr,
        ...
        pt_r2s, pt_best, pt_resids, pt_spec_ratio, pt_specs, pt_ext = full_eval_model(
            pt_model,
            dls,
            audios,
            1 / sr,
