...
def run_experiments(
    ...
    if not os.path.isfile(gdp):
        ...
        gabo_model = model_cv_lambdas(
            dls,
            1 / sr,
            nEpochs=100,
            model_path=gabo_path,
            save_freq=save_freq,
            tau=tau,
            lr=lr,
            n_kernels=n_kernels,
        ...
        gabo_r2s, gabo_best, gabo_resids, gabo_spec_ratio, gabo_specs, gabo_ext = (
            full_eval_model(
                gabo_model,
                dls,
                audios,
    ...
    if not os.path.isfile(sdp):
        ...
        dls = get_loaders(
            np.vstack(stacks)[:, :, None], cv=True, train_size=0.6, seed=seed
        )
        tau = 1 / sr_stack
        stack_path = os.path.join(synth_model_path, "stackies")
        stack_model = model_cv_lambdas(
            dls,
            1 / sr_stack,
            nEpochs=100,
    ...
    if not os.path.isfile(cdp):
        ...
        # print(coen_data[4])
        adult_model, *_ = load_model(
            adult_zf_model_path, kernel_type="full_poly"
        )  # here, let's use use cv_better_weighed_adultzf_92 0.05
        coen_r2s, coen_best, coen_resids, coen_spec_ratio, coen_specs, coen_ext = (
            full_eval_model(
                adult_model,
                None,
                coen_data[1],
    ...
    full_ax_dict["d2_stack_real"].plot(
        stack_best[0][0], stack_best[0][1], color="tab:blue"
    )
    ylim_real_s = full_ax_dict["d2_stack_real"].get_ylim()
    full_ax_dict["d2_stack_pred"].plot(
        stack_best[1][0], stack_best[1][1], color="tab:orange"
    )
    ...
    full_ax_dict["spec_stack_real"].imshow(
        stack_specs[0], origin="lower", aspect="auto", extent=stack_ext
    )
    full_ax_dict["spec_stack_emp"].imshow(
        stack_specs[1], origin="lower", aspect="auto", extent=stack_ext
    )
    full_ax_dict["spec_stack_md2"].imshow(
        stack_specs[2], origin="lower", aspect="auto", extent=stack_ext
    )
    full_ax_dict["spec_stack_m"].imshow(
        stack_specs[3], origin="lower", aspect="auto", extent=stack_ext
    )
    ...
    full_ax_dict["spec_g_real"].imshow(
        gabo_specs[0], origin="lower", aspect="auto", extent=gabo_ext
    )
    full_ax_dict["spec_g_emp"].imshow(
        gabo_specs[1], origin="lower", aspect="auto", extent=gabo_ext
    )
    full_ax_dict["spec_g_md2"].imshow(
        gabo_specs[2], origin="lower", aspect="auto", extent=gabo_ext
    )
    full_ax_dict["spec_g_m"].imshow(
        gabo_specs[3], origin="lower", aspect="auto", extent=gabo_ext
    )
    ...
    full_ax_dict["spec_c_real"].imshow(
        coen_specs[0], origin="lower", aspect="auto", extent=coen_ext
    )
    full_ax_dict["spec_c_emp"].imshow(
        coen_specs[1], origin="lower", aspect="auto", extent=coen_ext
    )
    full_ax_dict["spec_c_md2"].imshow(
        coen_specs[2], origin="lower", aspect="auto", extent=coen_ext
    )
    full_ax_dict["spec_c_m"].imshow(
        coen_specs[3], origin="lower", aspect="auto", extent=coen_ext
    )
