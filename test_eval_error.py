...
def test_eval_error():
    ...
    audios,sr = get_segmented_audio(audio_path,seg_path,audio_subdir=audio_subdir,\
                                    ...
                                    audio_type=audio_filetype,seg_type='.txt',max_pairs=250,seed=seed)

    dls = get_loaders(np.vstack(audios),cv = True,train_size=0.6,seed=seed)

    model_path_full_poly = model_path + f'/kernelborous_poly_end_to_end_lambda_0.75'
    save_loc_poly = model_path_full_poly + '/checkpoint_100.tar'


    kernel = fullPolyModule(nTerms=n_kernels,device='cuda',x_dim=1,z_dim=2,activation = lambda x: x,lam=0.75,trend_filtering=True)
    reg_weights=True    
    ...
    tl,vl,full_model_poly,full_opt_poly = train(full_model_poly,full_opt_poly,loss_fn=lambda y,yhat: sse(yhat,y,reduction='mean'),\
                        ...
                        dt = 1/sr,vis_freq=100,smoothing=False,\
    ...
    full_model_poly_loaded,full_opt_poly_loaded,full_scheduler_poly = load_model(save_loc_poly,kernel_type='full_poly')

    for repeat1 in range(10):
        (train_mu,test_mu),(train_sd,test_sd) = eval_model_error(dls,full_model_poly_loaded,dt=1/sr)
        (train_mu,test_mu),(train_sd,test_sd) = eval_model_error(dls,full_model_poly_loaded,dt=1/sr,comparison='test')
