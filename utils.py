...
def deriv_approx_dy(y, pad=True):
    ...
    if pad:
        y = np.concatenate(
            [np.flip(y[:, :4, :], axis=1), y, np.flip(y[:, -4:, :], axis=1)], axis=1
        )
    assert L >= 5, print(
        f"y is not long enough to approximate with 9 points! needs 9 points, has {L}"
    )
    return (
        3 * y[:, :-8, :]
        - 32 * y[:, 1:-7, :]
...
def get_spec(
    ...
    a, t = (
        audio[
            np.searchsorted(audiotimes, onset - shoulder) : np.searchsorted(
                audiotimes, offset + shoulder
            )
        ],
        audiotimes[
            np.searchsorted(audiotimes, onset - shoulder) : np.searchsorted(
    ...
    if interp:
        interp = RegularGridInterpolator(
            (fAx, tAx), Sx, bounds_error=True, fill_value=-1e10
        )
