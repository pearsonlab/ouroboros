...
def run_model(
    ...
    d_state=1,
    lam=1.5,
    mult_factor_kernels=4,
):

    d, d1, d2 = gen_stacks(
        n_samples, alpha=alpha, theta=theta, sample_rate=sample_rate, noise_sd=noise_sd
    )
    ...
    model_path_full_poly = model_path + "/kernelboros_poly_toy_end_to_end"
    ...
    if os.path.isfile(save_loc_poly):
        ...
        full_model_poly, full_opt_poly, full_scheduler_poly = load_model(
    ...
    (train_mu_poly, test_mu_poly), (train_sd_poly, test_sd_poly) = eval_model_error(
        dls, full_model_poly, dt=1 / sr, comparison="test"
    )
    assess_kernels(
        dls["train"], full_model_poly, dt=1 / sr, saveDir=model_path_full_poly
    )
    print("training comparison models end to end: then rbf kernel")
    kernel = simpleGaussModule(
        nTerms=mult_factor_kernels * n_kernels,
        device="cuda",
        x_dim=1,
        z_dim=2,
        activation=lambda x: x,
    ...
    full_model_gauss = rkhs_ouroboros(
        d_data=1,
        n_layers=n_layers,
        d_state=d_state,
        d_conv=d_conv,
    ...
    full_opt_gauss = Adam(full_model_gauss.parameters(), lr=lr)
    full_scheduler_gauss = ReduceLROnPlateau(
        full_opt_gauss, factor=0.5, patience=max(nEpochs // 25, 2), min_lr=1e-10
    )
    model_path_full_gauss = model_path + "/kernelborous_gauss_toy_end_to_end"
    save_loc_gauss = model_path_full_gauss + "/checkpoint_100.tar"
    if os.path.isfile(save_loc_gauss):
        # print(f'loading_checkpoint at {save_loc}')
        full_model_gauss, full_opt_gauss, full_scheduler_gauss = load_model(
            save_loc_gauss, kernel_type="gauss"
        )

    else:
        tl, vl, full_model_gauss, full_opt_gauss = train(
            full_model_gauss,
            full_opt_gauss,
            loss_fn=lambda y, yhat: sse(yhat, y, reduction="mean"),
            loaders=dls,
            scheduler=full_scheduler_gauss,
            nEpochs=nEpochs,
            val_freq=1,
    ...
    (train_mu_rbf, test_mu_rbf), (train_sd_rbf, test_sd_rbf) = eval_model_error(
        dls, full_model_gauss, dt=1 / sr, comparison="test"
    )
    assess_kernels(
        dls["train"], full_model_poly, dt=1 / sr, saveDir=model_path_full_gauss
    )

    # (train_coef,test_coef),(train_coef_sd,test_coef_sd) = eval_model_integration(dls,full_model,dt=1/sr,n_segs=25,st=0)

    r2_plot(
        [train_mu_poly, test_mu_poly, train_mu_rbf, test_mu_rbf],
