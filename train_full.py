...
def run_model(
    ...
    if not only_full:
        ...
        full_opt = Adam(full_model.parameters(), lr=lr)
        full_scheduler = ReduceLROnPlateau(
            full_opt, factor=0.75, patience=5, min_lr=1e-10
        )
        model_path_full = model_path_simple + f"/simpleborous_{voctype}_with_kernel"
        save_loc = model_path_full + "/checkpoint_100.tar"
        if os.path.isfile(save_loc):
            # print(f'loading_checkpoint at {save_loc}')
            full_model, full_opt, full_scheduler = load_model(
                save_loc, kernel_type=kernel_type
            )
