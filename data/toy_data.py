...
def gen_fm_data(
    n_samples, sample_rate=44100, frequency=5000, sample_length=0.3, sweep_rate=1000
):
...
def make_harmonic_stack(fundamental_freq, n_harmonics, t):
    ...
    for harm in range(1, n_harmonics + 1):
        ...
        d2_signal += (
            -((fundamental_freq * harm) ** 2)
            * np.sin(2 * np.pi * t * fundamental_freq * harm)
            * A
            / harm
        )
        d_signal += (
            (fundamental_freq * harm)
            * np.cos(2 * np.pi * t * fundamental_freq * harm)
            * A
            / harm
        )
...
def gen_stacks(n_samples, alpha=8.0, theta=2.0, sample_rate=44100, noise_sd=0.005):
...
descriptions = {
    "3": "400Hz: (1) spike train, constant interval",
