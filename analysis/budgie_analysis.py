...
def load_spikes_functions(
    datapath: str,
    model: object,
    padding: float = 0.1,
    model_smooth: float = 0.001,
    spike_smooth: float = 0.005,
):
    """
    ...
    for onset, offset in tqdm(onoffs, desc="getting neural and behavioral reps"):
        ...
        x, dxdt = (
            torch.from_numpy(x).to(model.device).to(torch.float32),
            torch.from_numpy(dxdt).to(model.device).to(torch.float32),
        )
        with torch.no_grad():
        ...
        omega, gamma = (
            gaussian_filter(omega, sigma=bin_func_smooth),
            gaussian_filter(gamma, sigma=bin_func_smooth),
        )
        ...
        spikebins = np.arange(
            onset - on_pad / sr,
            offset + off_pad / sr + spike_bin_size / 2,
            spike_bin_size,
        )
        ...
        for s in spiketimes:
            binned_spikes, _ = np.histogram(
                s[(s >= (onset - on_pad / sr)) * (s <= (offset + off_pad / sr))],
                bins=spikebins,
            )
...
def spike_function_design(
    ...
    for onset, offset in tqdm(onoffs, desc="getting neural and behavioral reps"):
        ...
        omega_spl, gamma_spl = (
            make_interp_spline(omega_t, omega),
            make_interp_spline(omega_t, gamma),
        )
...
def train_sliding_window_regression(
    datapath,
    model,
    model_smooth=0.001,
    spike_smooth=0.005,
    ...
    r2s_omega_null, r2s_gamma_null = (
        {"train": [], "test": []},
        {"train": [], "test": []},
    )
    r2s_omega_hardnull, r2s_gamma_hardnull = (
        {"train": [], "test": []},
        {"train": [], "test": []},
    )
    for run in range(1, n_runs + 1):
        ...
        if not os.path.isfile(model_path):
            ...
            if not os.path.isfile(data_path):
                print("loading data...")
                omegas, gammas, spike_designs, null_designs = spike_function_design(
                    datapath,
                    model,
                    model_smooth=model_smooth,
                    spike_smooth=spike_smooth,
                    win_length=win_length,
                    seed=seed,
                )

                data_dict = {
                    "spikes": spike_designs,
                    "nulls": null_designs,
                    "omegas": omegas,
                    "gammas": gammas,
                }
            ...
            spike_designs_stacked = np.vstack(
                [x.reshape(x.shape[0], -1) for x in spike_designs]
            )
            null_designs_stacked = np.vstack(
                [x.reshape(x.shape[0], -1) for x in null_designs]
            )
            ...
            order = gen.choice(
                len(spike_designs_stacked), len(spike_designs_stacked), replace=False
            )
            train_inds, test_inds = order[:n_train], order[n_train:]

            train_x, test_x = (
                spike_designs_stacked[train_inds],
                spike_designs_stacked[test_inds],
            )
            train_null, test_null = (
                null_designs_stacked[train_inds],
                null_designs_stacked[test_inds],
            )
            ...
            train_order_hardnull = gen.choice(
                len(train_om), len(train_om), replace=False
            )
            test_order_hardnull = gen.choice(len(test_om), len(test_om), replace=False)

            train_om_hardnull, test_om_hardnull = (
                train_om[train_order_hardnull],
                test_om[test_order_hardnull],
            )
            train_gam_hardnull, test_gam_hardnull = (
                train_om[train_order_hardnull],
                test_om[test_order_hardnull],
            )
            ...
            train_r2_om, train_r2_gam = (
                om_lr.score(train_x, train_om),
                gam_lr.score(train_x, train_gam),
            )

            test_r2_om, test_r2_gam = (
                om_lr.score(test_x, test_om),
                gam_lr.score(test_x, test_gam),
            )

            train_r2_om_null, train_r2_gam_null = (
                om_lr_null.score(train_null, train_om),
                gam_lr_null.score(train_null, train_gam),
            )

            test_r2_om_null, test_r2_gam_null = (
                om_lr_null.score(test_null, test_om),
                gam_lr_null.score(test_null, test_gam),
            )

            train_r2_om_hardnull, train_r2_gam_hardnull = (
                om_lr_hardnull.score(train_null, train_om_hardnull),
                gam_lr_hardnull.score(train_x, train_gam_hardnull),
            )

            test_r2_om_hardnull, test_r2_gam_hardnull = (
                om_lr_hardnull.score(test_x, test_om_hardnull),
                gam_lr_hardnull.score(test_x, test_gam_hardnull),
            )
            ...
            model_dict = {
                "real": {
                    "omega": {
                        "model": om_lr,
                        "train r2": train_r2_om,
                        "test r2": test_r2_om,
                    },
                    "gamma": {
                        "model": gam_lr,
                        "train r2": train_r2_gam,
                        "test r2": test_r2_gam,
                    },
                },
                "null": {
                    "omega": {
                        "model": om_lr_null,
                        "train r2": train_r2_om_null,
                        "test r2": test_r2_om_null,
                    },
                    "gamma": {
                        "model": gam_lr_null,
                        "train r2": train_r2_gam_null,
                        "test r2": test_r2_gam_null,
                    },
                },
                "hard null": {
                    "omega": {
                        "model": om_lr_hardnull,
                        "train r2": train_r2_om_hardnull,
                        "test r2": test_r2_om_hardnull,
                    },
                    "gamma": {
                        "model": gam_lr_hardnull,
                        "train r2": train_r2_gam_hardnull,
                        "test r2": test_r2_gam_hardnull,
                    },
                },
            }
            ...
            om_lr, train_r2_om, test_r2_om = (
                model_dict["real"]["omega"]["model"],
                model_dict["real"]["omega"]["train r2"],
                model_dict["real"]["omega"]["test r2"],
            )
            gam_lr, train_r2_gam, test_r2_gam = (
                model_dict["real"]["gamma"]["model"],
                model_dict["real"]["gamma"]["train r2"],
                model_dict["real"]["gamma"]["test r2"],
            )

            om_lr_null, train_r2_om_null, test_r2_om_null = (
                model_dict["null"]["omega"]["model"],
                model_dict["null"]["omega"]["train r2"],
                model_dict["null"]["omega"]["test r2"],
            )
            gam_lr_null, train_r2_gam_null, test_r2_gam_null = (
                model_dict["null"]["gamma"]["model"],
                model_dict["null"]["gamma"]["train r2"],
                model_dict["null"]["gamma"]["test r2"],
            )

            om_lr_hardnull, train_r2_om_hardnull, test_r2_om_hardnull = (
                model_dict["hard null"]["omega"]["model"],
                model_dict["hard null"]["omega"]["train r2"],
                model_dict["hard null"]["omega"]["test r2"],
            )
            gam_lr_hardnull, train_r2_gam_hardnull, test_r2_gam_hardnull = (
                model_dict["hard null"]["gamma"]["model"],
                model_dict["hard null"]["gamma"]["train r2"],
                model_dict["hard null"]["gamma"]["test r2"],
            )
            if run == 1:
                with open(data_path, "rb") as f:
                    data_dict = pickle.load(f)
        ...
        print(
            f"Omega null performance train: {train_r2_om_null:.4f}, test: {test_r2_om_null:.4f}"
        )
        print(
            f"Gamma null performance train: {train_r2_gam_null:.4f}, test: {test_r2_gam_null:.4f}"
        )

        print(
            f"Omega hard null performance train: {train_r2_om_hardnull:.4f}, test: {test_r2_om_hardnull:.4f}"
        )
        print(
            f"Gamma hard null performance train: {train_r2_gam_hardnull:.4f}, test: {test_r2_gam_hardnull:.4f}"
        )
        ...
        max_max = 0.01  # max(max_gam,max_om)

        ax = plt.gca()
        g = ax.matshow(
            weights_gam, aspect="auto", vmin=-max_max, vmax=max_max, cmap="RdBu_r"
        )
        ...
        if save_path != "":
            plt.savefig(
                os.path.join(save_path, f"gamma_weights_{run}.svg"), transparent=True
            )
        plt.show()
        plt.close()
        ...
        ax = plt.gca()
        g = ax.matshow(
            weights_om, aspect="auto", vmin=-max_max, vmax=max_max, cmap="RdBu_r"
        )
        ax.set_ylabel("Neuron number")
        ax.set_xlabel("Lag (ms)")
        ax.set_xticks(range(0, win_length, win_length // 4))
        ax.set_xticklabels(
            range(-win_length, 0, win_length // 4)
        )  # [-20,-15,-10,-5,-1])
        ...
        if save_path != "":
            plt.savefig(
                os.path.join(save_path, f"omega_weights_{run}.svg"), transparent=True
            )
        plt.show()
        plt.close()
        ...
        for jj, c in enumerate(choices):
            ...
            pred_gamma, pred_omega = (
                gam_lr.predict(spikes.reshape(spikes.shape[0], -1)),
                om_lr.predict(spikes.reshape(spikes.shape[0], -1)),
            )
            pred_gamma_null, pred_omega_null = (
                gam_lr_hardnull.predict(spikes.reshape(spikes.shape[0], -1)),
                om_lr_hardnull.predict(spikes.reshape(spikes.shape[0], -1)),
            )
            ...
            plt.savefig(
                os.path.join(save_path, f"model_{run}_recon_{jj}.svg"), transparent=True
            )
            plt.show()
            plt.close()
    ...
    mu_omega, mu_gamma = (
        np.nanmean(weight_mats_omega, axis=0),
        np.nanmean(weight_mats_gamma, axis=0),
    )
    max_gam = np.amax(np.abs(mu_gamma))
    max_om = np.amax(np.abs(mu_omega))
    max_max = 0.01  # max(max_gam,max_om)
    fig, (ax1, ax2, ax3) = plt.subplots(
        nrows=1, ncols=3, figsize=(15, 7), width_ratios=(7, 7, 1)
    )
    g = ax1.matshow(
        weights_gam,
        aspect="auto",
    ...
    if save_path != "":
        plt.savefig(
            os.path.join(save_path, "average_omega_gamma_weights.svg"), transparent=True
        )
    plt.show()
    plt.close()
