...
def integrate(
    ...
    omega, gamma, weighted_kernels, states = model.get_funcs(
        x[:1, :, :], dt, scaled=scaled
    )
...
def get_model_fncs(model, audios, dt, smoothing=True, smooth_len=0.005):
    ...
    for x in audios:
        ...
        # dx: dx_4dt,dx_5dt,dx_6dt,..., dx_(l-4)dt
        xddot = (
            torch.from_numpy(deriv_approx_d2y(x)).to(model.device).to(torch.float32)
            / dt**2
        )
        ...
        omega, gamma, weighted_kernel, weight, states = model.get_funcs(
            x, xdot, dt, scaled=True, smoothing=smoothing
        )

        omega, gamma, weighted_kernel = (
            omega.detach().cpu().numpy().squeeze(),
            gamma.detach().cpu().numpy().squeeze(),
        ...
        weight = (
            ...
            .cpu()
            .numpy()
            .reshape([weight.shape[0], weight.shape[1], -1])
            .squeeze()
        )
...
def get_marmo_fncs(
    model, audio_location, seg_location, marmo_name, voctype="", seed=None
):

    audio_location = os.path.join(
        audio_location, marmo_name, "wavfiles", "synchro_cleaned"
    )
    ...
    audios, sr = get_segmented_audio(
        ...
        envelope=False,
        context_len=0.15,
        audio_type=f"{voctype}_cleaned.wav",
        seg_type=f"{voctype}.txt",
        max_pairs=3000,
        seed=seed,
        full_vocs=True,
        extend=False,
...
def assess_variability(
    ...
    sd_gammas = np.nanstd(
        gammas, axis=0
    )  # /np.abs(np.nanmean(mu_gammas)) # as portion of mean mean!!
    sd_omegas = np.nanstd(
        omegas, axis=0
    )  # /np.abs(np.nanmean(mu_omegas)) # as portion of mean mean!!
