...
def get_spec_ava(audio: np.ndarray, p: dict) -> Tuple[np.ndarray, float, np.ndarray]:
    """
    Get a spectrogram.
...
def softmax(arr: np.ndarray, t: float = 0.5) -> np.ndarray:
    """softmax along first array dimension, not numerically stable. from AVA"""
...
def get_onsets_offsets(audio, p, return_traces=False):
    ...
    if len(audio) < p["nperseg"]:
        if return_traces:
            return [], [], None
        return [], []
    spec, dt, _ = get_spec_ava(audio, p)
