...
def run_model(
    ...
    for m in range(n_models):
        ...
        model_path_full = (
            model_path
            + f"/kernelborous_{voctype}_trendfiltering_{trend_level}_alpha_{alpha}_kernel_{kernel_type}_nkernels_{n_kernels}_model_{m}"
        )
        ...
        if os.path.isfile(save_loc):
            ...
            sd = {"ouroboros": model.state_dict(), "opt": opt.state_dict()}
        ...
        (train_coef, test_coef), (train_coef_sd, test_coef_sd) = eval_model_integration(
            dls, model, dt=1 / sr, n_segs=25, st=0
        )
    ...
    plt.savefig(
        os.path.join(
            model_path,
            f"train_test_repro_error_kernel_{kernel_type}_nkernels_{n_kernels}.svg",
        )
    )
    plt.close()
    ...
    plt.savefig(
        os.path.join(
            model_path,
            f"train_test_repro_int_coef_kernel_{kernel_type}_nkernels_{n_kernels}.svg",
        )
    )
    plt.close()
