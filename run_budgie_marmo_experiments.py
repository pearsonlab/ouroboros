...
def run_experiments(
    ...
    if zf:
        ...
        audios, sr = get_segmented_audio(
            ...
            context_len=0.15,
            audio_type="_cleaned.wav",
            seg_type="_syllableTimeStamp.txt",
            max_pairs=3000,
            seed=seed,
        )
        dls = get_loaders(np.vstack(audios), cv=True, train_size=0.6, seed=seed)
        tau = 1 / sr
        zf_model = model_cv_lambdas(
            dls,
            1 / sr,
            nEpochs=400,
            model_path=zf_model_path,
            lr=lr,
            tau=tau,
            n_kernels=n_kernels,
        )
        ...
        if os.path.isfile(os.path.join(zf_model_path, "eval_data.pkl")):
            ...
            zf_r2s, zf_best, zf_resids, zf_spec_ratio, zf_specs, zf_ext = (
                full_eval_model(
                    zf_model,
                    dls,
                    audios,
    ...
    if marmoset:
        ...
        ### since we're training across all marmosets, ensure we have a few
        # marmos per day
        marmos = glob.glob(os.path.join(marmo_data_path, "s6*"))
        audios = []
        for marmo in marmos:
            print(f"loading for {marmo}")
            marmo_audio_path, marmo_seg_path = marmo, marmo
            marmo_audio_subdir = "wavfiles/synchro_cleaned"
            marmo_seg_subdir = "wavfiles"

            a, sr = get_segmented_audio(
                marmo_audio_path,
                marmo_seg_path,
