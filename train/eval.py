...
def assess_integration_torch(
    ...
    print(
        "function does not currently work. leave it alone. if you want to evaluate integration,"
    ...
    xddot = (
        torch.from_numpy(deriv_approx_d2y(x)).to(model.device).to(torch.float32) / dt**2
    )
    ...
    omega, gamma, weighted_kernels, weights, states = model.get_funcs(
        x, xdot, dt, scaled=True, smoothing=smoothing
    )

    omega, gamma, weighted_kernels = (
        omega.detach().cpu().numpy().squeeze(),
        gamma.detach().cpu().numpy().squeeze(),
    ...
    weights = (
        ...
        .cpu()
        .numpy()
        .reshape([weights.shape[0], weights.shape[1], -1])
        .squeeze()
    )
    # print(weights.shape)
    # assert False
    ...
    t_steps = np.arange(0, L * dt + dt / 2, dt)[:L]
    t_eval = torch.arange(0, int_length, dt / oversample_prop, device=model.device)[
        : int_length_samples * oversample_prop
    ]
    dt_used = dt / oversample_prop

    omegaTerp = make_interp_spline(
        t_steps, omega
    )  # lambda t: np.interp(t,t_steps,omega)
    gammaTerp = make_interp_spline(
        t_steps, gamma
    )  # lambda t: np.interp(t,t_steps,gamma)
    weighted_kernelsTerp = lambda t: np.interp(t, t_steps, weighted_kernels)
    weightsTerp = [
        make_interp_spline(t_steps, weights[:, ii]) for ii in range(weights.shape[1])
    ]  # [lambda t,n=ii: np.interp(t,t_steps,weights[:,n]) for ii in range(weights.shape[1])]
    ...
    xddot_terp = make_interp_spline(
        t_steps, xddot.detach().cpu().numpy().squeeze()
    )  # lambda t: np.interp(t,t_steps,xddot.detach().cpu().numpy().squeeze())
    ...
    def dz_true(t, z):
        ...
        if strategy == "interp":
            dz2_step = (
                torch.from_numpy(np.array([xddot_terp(t)]))
                .to(model.device)
                .to(torch.float32)
            )
        else:
            dz2_step = xddot[
                0, s_ind, 0
            ]  # torch.from_numpy(xddot[s_ind]])).to(model.device).to(torch.float32)
    ...
    yhat = (
        yhat.detach().cpu().numpy().squeeze() / model.tau**2
    )  # fix to match new scaling on model output
    assert np.sum(np.isnan(yhat)) == 0, print(
        np.sum(np.isnan(yhat)) / np.prod(yhat.shape)
    )
    if plot:
        ax = plt.gca()
    ...
    def dz_hat1(t, z):
        ...
        if strategy == "interp":
            dz2_step = (
                torch.from_numpy(np.array([yhat_terp(t)]))
                .to(model.device)
                .to(torch.float32)
            )
        else:
            dz2_step = (
                torch.from_numpy(np.array([yhat[s_ind]]))
                .to(model.device)
                .to(torch.float32)
            )
    ...
    def dz_hat2(t, z):
        ...
        if strategy == "interp":
            ...
            weights_step = np.reshape(
                np.stack([terp(t) for terp in weightsTerp]), [1, 1, P, P]
            )
        else:
        ...
        weighted_kernels_step = model.kernel.forward_given_weights_numpy(
            z[None, :].detach().cpu().numpy(), weights_step
        ).squeeze()
        ...
        dz2 = (
            torch.from_numpy(
                np.array(
                    [-(omega_step**2) * z1 - gamma_step * z2 - weighted_kernels_step]
                )
            )
            .to(model.device)
            .to(torch.float32)
    ...
    y1 = odeint_adjoint(
        dz_true, z0, t_eval, adjoint_params=(), method=method, options=dict()
    ).transpose(0, 1)
    print("done! integrating model dz2...")
    y2 = odeint_adjoint(
        dz_hat1, z0, t_eval, adjoint_params=(), method=method, options=dict()
    ).transpose(0, 1)
    print("done! integrating model functions....")
    s = time.time()
    y3 = odeint_adjoint(
        dz_hat2, z0, t_eval, adjoint_params=(), method=method, options=dict()
    ).transpose(0, 1)
    ...
    xTrue, y1_corr, y2_corr, y3_corr = (
        xTrue[:min_len],
        y1_corr[:min_len],
        y2_corr[:min_len],
        y3_corr[:min_len],
    )
    truspec, ittr, iftr, *_ = get_spec(
        xTrue,
        int(round(1 / dt)),
        onset=0,
        offset=xTrue.shape[0] * dt,
        shoulder=0.0,
        interp=False,
        win_len=1028,
        normalize=False,
    ...
    modelspec, *_ = get_spec(
        y3_corr,
        int(round(1 / dt)),
        onset=0,
        offset=y3.squeeze().shape[0] * dt,
        shoulder=0.0,
        interp=False,
        win_len=1028,
        normalize=False,
    ...
    if plot:
        ...
        axs[0].imshow(
            ...
            origin="lower",
            aspect="auto",
            extent=[ittr[0], ittr[-1], iftr[0], iftr[-1]],
        )
        axs[1].imshow(
            intspec,
            origin="lower",
    ...
    return (
        (trueErr, hat1Err, hat2Err),
        (y1, y2, y3),
        (y1_corr, y2_corr, y3_corr),
        (truspec, intspec, int2spec, modelspec, (ittr, iftr)),
...
def eval_model_error(
    ...
    for idx, batch in enumerate(dls[comparison]):
        with torch.no_grad():
            ...
            tot = sst(
                y, reduction="none"
            )  # ((y - y.mean(dim=1,keepdim=True))**2).sum(dim=1)
            tot = tot.detach().cpu().numpy().squeeze()
...
def assess_kernels(dataloader, model, dt, saveDir=""):
    ...
    weight_samples = np.random.choice(
        len(dataloader), max(len(dataloader) // 5 + 1, 1), replace=False
    )
    ...
    for ii, batch in enumerate(dataloader):
        with torch.no_grad():
            ...
            x = x.to("cuda").to(torch.float32)
            dxdt = dxdt.to("cuda").to(torch.float32)
            _, _, _, weights, _ = model.get_funcs(
                x, dxdt, dt, scaled=True, smoothing=True
            )
        ...
        assert weights_sample.shape[0] == ydy_batch.shape[0], print(
            weights_sample.shape, ydy_batch.shape
        )
        assert weights_sample.shape[1] == ydy_batch.shape[1], print(
            weights_sample.shape, ydy_batch.shape
        )
        ...
        if ii in weight_samples:
            ...
            plt.savefig(
                os.path.join(saveDir, f"kernel_weights_{ii}_{batch_sample}.png")
            )
            plt.close()
    ...
    g = ax.imshow(
        ...
        origin="lower",
        aspect="auto",
        extent=[ypts[0], ypts[-1], dypts[0], dypts[-1]],
    )
    ax.set_xlabel("y")
    ax.set_ylabel("dy")
    cb = plt.colorbar(g, ax=ax)
    cb.set_label(
        "log(abs(Kernel value)) * sign(kernel value)", rotation=270, labelpad=40
    )
...
def eval_model_integration(dls, model, dt, n_segs=100, st=0.05, comparison="val"):
    ...
    test_choices = np.random.choice(
        len(dls[comparison].dataset), max_segs, replace=False
    )

    train_errs, test_errs = [], []
    for train_ind, test_ind in tqdm(
        zip(train_choices, test_choices), desc="Integrating samples...", total=max_segs
    ):
        ...
        train_x, test_x = (
            train_x.view(1, len(train_x), 1),
            test_x.view(1, len(test_x), 1),
        )
        train_dxdt, test_dxdt = (
            train_dxdt.view(1, len(train_dxdt), 1),
            test_dxdt.view(1, len(test_dxdt), 1),
        )

        test_y_modelint, *_ = model.integrate(
            test_x.to("cuda").to(torch.float32),
        ...
        train_y_modelint, *_ = model.integrate(
            train_x.to("cuda").to(torch.float32),
        ...
        train_x, test_x = (
            train_x.detach().cpu().numpy().squeeze(),
            test_x.detach().cpu().numpy().squeeze(),
        )
        # train_y, test_y = train_y.detach().cpu().numpy().squeeze()[3:-5],test_y.detach().cpu().numpy().squeeze()[3:-5]
        L = len(train_x)
        L_train, L_test = (
            min(L, train_y_modelint.shape[1]),
            min(L, test_y_modelint.shape[1]),
        )

        z0_test, z0_train = (
            np.hstack([test_x[0], test_dxdt[0] / dt]),
            np.hstack([train_x[0], train_dxdt[0] / dt]),
        )
        ...
        test_yint = remove_rm(
            integrate_d2y(test_dx2, t_samples=t, init_cond=z0_test), rm_length=5
        )[0, start:L_test]
        train_yint = remove_rm(
            integrate_d2y(train_dx2, t_samples=t, init_cond=z0_train), rm_length=5
        )[0, start:L_train]
        train_y_modelint = remove_rm(train_y_modelint.squeeze(), rm_length=5)[
            0, start:L_train
        ]
        test_y_modelint = remove_rm(test_y_modelint.squeeze(), rm_length=5)[
            0, start:L_test
        ]
...
def full_eval_model(
    ...
    if not use_results:
        ...
        if loaders == None:
            ...
            for o in original_data:
                ...
                # print(o.shape)
                dx2dt2 = (
                    torch.from_numpy(deriv_approx_d2y(o))
                    .to(model.device)
                    .to(torch.float32)
                )
                dxdt = (
                    torch.from_numpy(deriv_approx_dy(o))
                    .to(model.device)
                    .to(torch.float32)
                )
                # print(dt)
            ...
            with torch.no_grad():
                _, _, (_, test_r2s), (test_preds, test_reals, original_data) = (
                    eval_model_error(
                        loaders, model, dt, comparison="test", return_all=True
                    )
                )
    ...
    pix_err_real, pix_err_predd2, pix_err_predmod = (
        np.array(pix_err_real),
        np.array(pix_err_predd2),
        np.array(pix_err_predmod),
    )
    ...
    return (
        r2s,
        ((traj_xax, best_traj_real * (dt**2)), (traj_xax, best_traj_pred * (dt**2))),
        (res_xax, best_res),
        errs,
