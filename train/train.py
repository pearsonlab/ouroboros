...
def save_model(model:torch.nn.Module,opt:torch.optim.Optimizer,location:str,\
               n_layers:int=2,d_state:int=1,\
                d_conv:int=4,expand_factor:int=4,
                max_saved:int=5):
...
def load_model(location:str)->Tuple[nn.Module,
    ...
    try:
        ...
        kernel = fullPolyModule(nTerms=sd['n_kernel'],device='cuda',x_dim=1,z_dim=2,activation=lambda x: x,lam=1)
...
def train(model,optimizer,loss_fn,loaders,scheduler=None,
          nEpochs=100,val_freq=25,runDir='.',dt=1/44100,\
            ...
            start_epoch=0,model_info={},save_freq=0)->Tuple[list[float],
    ...
    for epoch in tqdm(range(start_epoch,nEpochs),desc='training model'):
        ...
        for idx,batch in enumerate(loaders['train'],start=epoch*len(loaders['train'])):
            ...
            if reg_weights:
                ...
                penalty =  (w *weights**2).sum(dim=(-1,-2,-3)).mean() # new (sum over weights, time), average over samples. equivalent to squared L2 norm
